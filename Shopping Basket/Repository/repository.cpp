...
void Repository::AddRepo(string size, string color, int price, int quantity, string photograph) {
    int position;
...
const vector<Coat>& Repository::GetDynArr() {
...
void Repository::DeleteEntireProductRepo(string size, string color, string photograph) {
    int position;
    position = CheckExistenceCoat(size, color, photograph);
    if(position != -1)
    {
...
void Repository::DeleteSoldOutProductRepo(string size, string color, string photograph) {
    int position;
    position = CheckExistenceCoat(size, color, photograph);
...
void Repository::UpdatePriceRepo(string size, string color, string photograph, int price) {
    int position;
    position = CheckExistenceCoat(size, color, photograph);
    if(position != -1)
    {
...
Coat Repository::getCoat(string size, string color, string photograph) {
    int position = CheckExistenceCoat(size, color, photograph);
    if(position != -1)
...
//vector<Coat> Repository::GetDynArr() {
//    return this->elements;
//}
